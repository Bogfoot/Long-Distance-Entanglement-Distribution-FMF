...
QBER_50 = 0.50
VISIBILITY_50 = 0.50
VISIBILITY_BELL = 1.0 / np.sqrt(2.0)

QBER_REFERENCE_LINES = [
    (QBER_11, "QBER 11%", "#1F77B4", "--"),
    (QBER_50, "QBER target 50%", "#2CA02C", ":"),
]
VISIBILITY_REFERENCE_LINES = [
    (VISIBILITY_50, "Visibility 50%", "#1F77B4", "--"),
    (VISIBILITY_BELL, "Visibility 70.7%", "#D62728", ":"),
]
...
def plot_qber_time_series(records, metadata) -> Path:
    ...
    axis.axhline(min(qber), color="#7F7F7F", linestyle=":", linewidth=1.0, label=f"lowest QBER {format_percent(min(qber), 2)}")
    for value, label, color, style in QBER_REFERENCE_LINES:
...
def plot_visibility_time_series(records, metadata) -> Path:
    ...
    for value, label, color, style in VISIBILITY_REFERENCE_LINES:
        axis.axhline(value, color=color, linestyle=style, linewidth=1.0, label=label)
...
def main() -> None:
    ...
    plot_qber_time_series(records, metadata)
    plot_visibility_time_series(records, metadata)
    plot_qber_by_window(records)
    plot_visibility_by_window(records)
    plot_qber_stability_monitor(records)
    plot_visibility_stability_monitor(records)
