...
class QuTAG:
    ...
    def __init__(self):
        ...
        # load DLL 
        self.qutools_dll = ctypes.windll.LoadLibrary(os.path.join(full_path, dll_name))
    ...
    def __declareAPI(self):
        ...
        # ------- tdcbase.h --------------------------------------------------------
        self.TDC_QUTAG_CHANNELS = self.qutools_dll.TDC_getChannelCount()
        ...
        self.qutools_dll.TDC_getChannelCount.argtypes = None
        self.qutools_dll.TDC_getChannelCount.restype = ctypes.c_int32
        self.qutools_dll.TDC_startCalibration.argtypes = None
        self.qutools_dll.TDC_startCalibration.restype = ctypes.c_int32
        self.qutools_dll.TDC_getCalibrationState.argtypes = [ctypes.POINTER(ctypes.c_int32)]
        ...
        self.qutools_dll.TDC_enableChannels.argtypes = [ctypes.c_int32,ctypes.c_int32]
        self.qutools_dll.TDC_enableChannels.restype = ctypes.c_int32
        self.qutools_dll.TDC_getChannelsEnabled.argtypes = [ctypes.POINTER(ctypes.c_int32),ctypes.POINTER(ctypes.c_int32)]
        ...
        self.qutools_dll.TDC_setChannelDelay.argtypes = [ctypes.c_int32,ctypes.c_int32]
        self.qutools_dll.TDC_setChannelDelay.restype = ctypes.c_int32
        self.qutools_dll.TDC_getChannelDelay.argtypes = [ctypes.c_int32,ctypes.POINTER(ctypes.c_int32)]
        self.qutools_dll.TDC_getChannelDelay.restype = ctypes.c_int32
        ...
        self.qutools_dll.TDC_getClockState.argtypes = [ctypes.POINTER(ctypes.c_int32), ctypes.POINTER(ctypes.c_int32)]
        self.qutools_dll.TDC_getClockState.restype = ctypes.c_int32
        
        self.qutools_dll.TDC_enableExternalClock.argtypes = [ctypes.c_int32]
        self.qutools_dll.TDC_enableExternalClock.restype = ctypes.c_int32
...
# Init --------------------------------------------------------------    
    def Initialize(self): 
        ...
        if (ans != 0):
            ...
            raise RuntimeError(f"This error again: {self.err_dict[ans]}") 
        return ans
...
# Device Info -------------------------------------------------------------
    ...
    def getChannelCount(self):
        ans = self.qutools_dll.TDC_getChannelCount()
        return ans
    ...
    def getClockState(self):
        ...
        uplink = ctypes.c_int32()
        ans = self.qutools_dll.TDC_getClockState(ctypes.byref(locked),ctypes.byref(uplink))

        return (locked.value,uplink.value)
        
    def enableExternalClock(self, enable):
        if enable:
            ena_value = 1
        else:
            ena_value = 0
        ans = self.qutools_dll.TDC_enableExternalClock(ena_value)
        if ans != 0: # "never fails"
            print("Error in TDC_enableExternalClock: "+self.err_dict[ans])
        return ans    
...
# Configure Channels ----------------------------------------------------------------
    ...
    def getChannelDelay(self, channel):
        delay = ctypes.c_int32()
        ans = self.qutools_dll.TDC_getChannelDelay(channel, ctypes.byref(delay))
        if (ans != 0):
            print("Error in TDC_getChannelDelays: " + self.err_dict[ans])
        return delay.value
        
    def setChannelDelay(self, channel, delays):
        ans = self.qutools_dll.TDC_setChannelDelay(int(channel), int(delays))
        if (ans != 0):
    ...
    def enableChannels(self, enStart, channels = False):
        self.TDC_QUTAG_CHANNELS = self.qutools_dll.TDC_getChannelCount()
        enStartBit = 0
        if enStart:
        	enStartBit = 1
        ...
        if channels:
            ...
            bitstring = ''		
            for k in range(1,self.TDC_QUTAG_CHANNELS):
                bitstring = '1' + bitstring
				
        channelMask = int(str(bitstring), 2)
        ans = self.qutools_dll.TDC_enableChannels(enStartBit,channelMask)
        if ans != 0:
    ...
    def getChannelsEnabled(self):
        self.TDC_QUTAG_CHANNELS = self.qutools_dll.TDC_getChannelCount()
        channelStart = ctypes.c_int32()
        channelMask = ctypes.c_int32()
        ans = self.qutools_dll.TDC_getChannelsEnabled(ctypes.byref(channelStart),ctypes.byref(channelMask))

        enabledChannels = ''
        mask = channelMask.value
        i=1
        while mask >  0:
            enabledChannels = str(mask % 2) + enabledChannels
            mask //= 2
            i += 1	
...
# Lifetime ----------------------------------------------------------
    ...
    def getLFTHistogram(self,channel,reset, lft):
        ...
        return (lft, tooBig.value, startevt.value, stopevt.value, expTime.value)
...
# HBT ---------------------------------------------------------------
    ...
    def getHBTParams(self):
        binWidth = ctypes.c_int32()
        binCount = ctypes.c_int32()
        ans = self.qutools_dll.TDC_getHbtParams(ctypes.byref(binWidth),ctypes.byref(binCount))
        if ans != 0:
